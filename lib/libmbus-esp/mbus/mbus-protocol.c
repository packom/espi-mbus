    ...
    if (frame)
    {
        switch (frame->type)
        {
            ...
            case MBUS_FRAME_TYPE_SHORT:
                ...
                {
                    snprintf(error_str, sizeof(error_str), "Unknown Control Code 0x%x", frame->control);
            ...
            default:
                snprintf(error_str, sizeof(error_str), "Unknown frame type 0x%x", frame->type);
        ...
        {
            snprintf(error_str, sizeof(error_str), "Invalid checksum (0x%x != 0x%x)", frame->checksum, checksum);
...
mbus_data_bin_decode(unsigned char *dst, const unsigned char *src, size_t len, size_t max_len)
{
    ...
    if (src && dst)
    {
        while((i < len) && ((pos+3) < max_len)) {
            pos += snprintf(&dst[pos], max_len - pos, "%x ", src[i]);
            i++;
        }
...
mbus_data_variable_medium_lookup(unsigned char medium)
{
    ...
    switch (medium)
    {
        ...
        // add more ...
        default:
            snprintf(buff, sizeof(buff), "Unknown medium (0x%x)", medium);
            break;
    }
...
mbus_vif_unit_lookup(unsigned char vif)
{
    ...
    switch (vif & MBUS_DIB_VIF_WITHOUT_EXTENSION) // ignore the extension bit in this selection
    {
        ...
        default:
            snprintf(buff, sizeof(buff), "Unknown (VIF=0x%x)", vif);
            break;
    }
...
mbus_data_error_lookup(int error)
{
    ...
    switch (error)
    {
        ...
        default:
            snprintf(buff, sizeof(buff), "Unknown error (0x%x)", error);
            break;
    }
...
mbus_vib_unit_lookup(mbus_value_information_block *vib)
{
    ...
    if (vib->vif == 0xFD || vib->vif == 0xFB) // first type of VIF extention: see table 8.4.4
    {
        ...
        {
            snprintf(buff, sizeof(buff), "Unrecognized VIF extension: 0x%x", vib->vife[0]);
        }
        return buff;
    }
    ...
    if (record)
    {
        ...
        switch (record->drh.dib.dif & MBUS_DATA_RECORD_DIF_MASK_DATA)
        {
            ...
            case 0x01: // 1 byte integer (8 bit)
                ...
                if (debug)
                    printf("%s: DIF 0x%x was decoded using 1 byte integer\n", __PRETTY_FUNCTION__, record->drh.dib.dif);
            ...
            case 0x02: // 2 byte (16 bit)
                ...
                else  // 2 byte integer
                {
                    ...
                    snprintf(buff, sizeof(buff), "%d", int_val);
                    if (debug)
                        printf("%s: DIF 0x%x was decoded using 2 byte integer\n", __PRETTY_FUNCTION__, record->drh.dib.dif);
            ...
            case 0x03: // 3 byte integer (24 bit)
                ...
                if (debug)
                    printf("%s: DIF 0x%x was decoded using 3 byte integer\n", __PRETTY_FUNCTION__, record->drh.dib.dif);
            ...
            case 0x04: // 4 byte (32 bit)
                    ...
                    ((record->drh.vib.vif == 0xFD) && (vife == 0x70)))
                {
                    ...
                    snprintf(buff, sizeof(buff), "%d-%d-%dT%d:%d:%d",
                                                 (time.tm_year + 1900),
                                                 (time.tm_mon + 1),
                                                  time.tm_mday,
                                                  time.tm_hour,
                ...
                if (debug)
                    printf("%s: DIF 0x%x was decoded using 4 byte integer\n", __PRETTY_FUNCTION__, record->drh.dib.dif);
            ...
            case 0x05: // 4 Byte Real (32 bit)
                ...
                if (debug)
                    printf("%s: DIF 0x%x was decoded using 4 byte Real\n", __PRETTY_FUNCTION__, record->drh.dib.dif);
            ...
            case 0x06: // 6 byte (48 bit)
                ...
                else  // 6 byte integer
                {
                    ...
                    snprintf(buff, sizeof(buff), "%d", long_long_val);
                }

                if (debug)
                    printf("%s: DIF 0x%x was decoded using 6 byte integer\n", __PRETTY_FUNCTION__, record->drh.dib.dif);
            ...
            case 0x07: // 8 byte integer (64 bit)
                ...
                if (debug)
                    printf("%s: DIF 0x%x was decoded using 8 byte integer\n", __PRETTY_FUNCTION__, record->drh.dib.dif);
            ...
            case 0x09: // 2 digit BCD (8 bit)
                ...
                if (debug)
                    printf("%s: DIF 0x%x was decoded using 2 digit BCD\n", __PRETTY_FUNCTION__, record->drh.dib.dif);
            ...
            case 0x0A: // 4 digit BCD (16 bit)
                ...
                if (debug)
                    printf("%s: DIF 0x%x was decoded using 4 digit BCD\n", __PRETTY_FUNCTION__, record->drh.dib.dif);
            ...
            case 0x0B: // 6 digit BCD (24 bit)
                ...
                if (debug)
                    printf("%s: DIF 0x%x was decoded using 6 digit BCD\n", __PRETTY_FUNCTION__, record->drh.dib.dif);
            ...
            case 0x0C: // 8 digit BCD (32 bit)
                ...
                if (debug)
                    printf("%s: DIF 0x%x was decoded using 8 digit BCD\n", __PRETTY_FUNCTION__, record->drh.dib.dif);
            ...
            case 0x0E: // 12 digit BCD (48 bit)
                ...
                if (debug)
                    printf("%s: DIF 0x%x was decoded using 12 digit BCD\n", __PRETTY_FUNCTION__, record->drh.dib.dif);
            ...
            default:
                ...
                snprintf(buff, sizeof(buff), "Unknown DIF (0x%x)", record->drh.dib.dif);
                break;
        }
...
long 
ICACHE_FLASH_ATTR
mbus_data_record_storage_number(mbus_data_record *record)
{
    int bit_index = 0;
    long result = 0;
    int i;
...
long 
ICACHE_FLASH_ATTR
mbus_data_record_tariff(mbus_data_record *record)
{
    int bit_index = 0;
    long result = 0;
    int i;
...
int 
ICACHE_FLASH_ATTR
mbus_data_record_device(mbus_data_record *record)
{
    int bit_index = 0;
    int result = 0;
    int i;
...
mbus_parse(mbus_frame *frame, unsigned char *data, size_t data_size)
{
    ...
    if (frame && data && data_size > 0)
    {
        ...
        if (parse_debug)
            printf("%s: Attempting to parse binary data [size = %u]\n", __PRETTY_FUNCTION__, data_size);
...
mbus_frame_data_parse(mbus_frame *frame, mbus_frame_data *data)
{
    ...
    if (direction == MBUS_CONTROL_MASK_DIR_S2M)
    {
        ...
        {
            snprintf(error_str, sizeof(error_str), "Unknown control information 0x%x", frame->control_information);
    ...
    switch (frame_data->type)
    {
        ...
        case MBUS_DATA_TYPE_VARIABLE:
            ...
            for (record = frame_data->data_var.record; record; record = record->next)
            {
                // pack DIF
                if (parse_debug)
                    printf("%s: packing DIF [%u]", __PRETTY_FUNCTION__, frame->data_size);
                ...
                // pack VIF
                if (parse_debug)
                    printf("%s: packing VIF [%u]", __PRETTY_FUNCTION__, frame->data_size);
                ...
                // pack data
                if (parse_debug)
                    printf("%s: packing data [%u : %u]", __PRETTY_FUNCTION__, frame->data_size, record->data_len);
                for (j = 0; j < record->data_len; j++)
                {
...
mbus_data_variable_print(mbus_data_variable *data)
{
    ...
    if (data)
    {
        ...
        for (record = data->record; record; record = record->next)
        {
            ...
            {
                printf("%s: VENDOR DATA [size=%u] = ", __PRETTY_FUNCTION__, record->data_len);
                for (j = 0; j < record->data_len; j++)
                {
...
mbus_hex_dump(const char *label, const char *buff, size_t len)
{
    ...
    fprintf(stderr, "[%s] %s (%03u):", timestamp, label, len);
...
mbus_data_variable_header_xml(mbus_data_variable_header *header)
{
    ...
    if (header)
    {
        ...
        len += snprintf(&buff[len], sizeof(buff) - len, "        <Status>%x</Status>\n", header->status);
        len += snprintf(&buff[len], sizeof(buff) - len, "        <Signature>%x%x</Signature>\n", header->signature[1], header->signature[0]);
...
mbus_data_variable_record_xml(mbus_data_record *record, int record_cnt, int frame_cnt, mbus_data_variable_header *header)
{
    ...
    if (record)
    {
        ...
        else
        {
            ...
            len += snprintf(&buff[len], sizeof(buff) - len,
                            "        <StorageNumber>%d</StorageNumber>\n",
                            mbus_data_record_storage_number(record));

            if ((tariff = mbus_data_record_tariff(record)) >= 0)
            {
                len += snprintf(&buff[len], sizeof(buff) - len, "        <Tariff>%d</Tariff>\n",
                                tariff);
...
mbus_data_fixed_xml(mbus_data_fixed *data)
{
    ...
    if (data)
    {
        ...
        len += snprintf(&buff[len], buff_size - len, "        <Status>%x</Status>\n", data->status);
        len += snprintf(&buff[len], buff_size - len, "    </SlaveInformation>\n\n");
